...
function useInView<T extends HTMLElement>() {
  ...
  useEffect(() => {
    ...
    if (!el) return;
    if (!revealObserver) { setInView(true); return; }
    const handler = () => setInView(true);
    el.addEventListener('inview', handler, { once: true });
    const mo = new MutationObserver(() => {
      if (el.dataset.inView === '1') { el.dispatchEvent(new Event('inview')); mo.disconnect(); }
    });
    mo.observe(el, { attributes: true, attributeFilter: ['data-in-view'] });
    revealObserver.observe(el);
    return () => { mo.disconnect(); el.removeEventListener('inview', handler); revealObserver!.unobserve(el); };
  }, []);
  return { ref, inView };
}
      ...
      <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${scrolled ? 'border-b border-slate-200/80 bg-white/90 backdrop-blur-lg shadow-sm' : 'bg-transparent'}`}>
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 sm:py-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <Link to="/login" className="text-sm font-600 text-slate-600 transition hover:text-brand-600">تسجيل الدخول</Link>
            <button onClick={() => setShowRequest(true)} className="btn-primary text-sm whitespace-nowrap">طلب خدمة <ArrowRight size={14} /></button>
          </div>
        </div>
      </header>
