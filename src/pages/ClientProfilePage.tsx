...
import { printHtml } from '@/lib/printExport';
import {
  VISIT_STATUS_LABELS,
...
export function ClientProfilePage() {
  ...
  <div style="text-align:center;">
    <img style="width:64px;height:64px;border-radius:12px;object-fit:cover;margin:0 auto 16px;display:block;" src="/756132783_2274483666620765_5282871515405515489_n.jpg" alt="ARKON" onerror="this.style.display='none'" />
    <div style="font-size:28px;font-weight:700;letter-spacing:2px;color:#0f172a;margin-bottom:4px;">ARKON</div>
    <div style="font-size:12px;color:#4f46e5;letter-spacing:3px;text-transform:uppercase;margin-bottom:32px;">Facility Management</div>
    <div style="border:2px solid #e2e8f0;border-radius:16px;padding:32px;max-width:420px;margin:0 auto;">
      <div style="font-size:22px;font-weight:700;color:#0f172a;margin-bottom:8px;">${clientName}</div>
      <div style="font-size:14px;color:#64748b;font-family:monospace;margin-bottom:24px;">رقم العميل: ${clientId}</div>
      <img style="width:300px;height:300px;margin:0 auto 16px;display:block;" src="${qrUrl}" alt="QR Code" />
      <div style="font-family:monospace;font-size:16px;color:#334155;background:#f1f5f9;border-radius:8px;padding:8px 16px;display:inline-block;margin-top:8px;">${codeValue}</div>
    </div>
    <div style="margin-top:32px;font-size:12px;color:#94a3b8;">تم الإنشاء بواسطة نظام ARKON — ${new Date().toLocaleDateString('ar-EG')}</div>
  </div>`;

    printHtml(`رمز QR - ${clientName}`, body);
  };
